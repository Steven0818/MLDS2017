...
class Effective_attention_model():
    ...
    def __init__(self,frame_steps=20, frame_feat_dim=4096, caption_steps=45, vocab_size=3000, dim_hidden=200, schedule_sampling_converge=500):
        ...
        ## word embedding to onehot param
        self.w_word_onehot = tf.get_variable("w_word_onehot", [dim_hidden, vocab_size], initializer=tf.contrib.layers.xavier_initializer(dtype=tf.float32))
        self.b_word_onehot = tf.get_variable("b_word_onehot", [vocab_size], initializer=tf.constant_initializer(0.0))
        ...
        for i in range(frame_steps):
            ...
            with tf.variable_scope('att_lstm'):
                if i > 0:
                    tf.get_variable_scope().reuse_variables()
                output1, self.att_state = att_lstm(frame_embedding[:,i,:], self.att_state)
            ##input shape of cap_lstm2: [batch_size, 2*dim_hidden]
            with tf.variable_scope('cap_lstm'):
                if i > 0:
                    tf.get_variable_scope().reuse_variables()
                output2, self.cap_state = cap_lstm(output1, self.cap_state)
            self.enc_lstm_outputs.append(output2)
        
        ## (batch_size,frame_step,dim_hidden)
        self.enc_lstm_outputs = tf.reshape(tf.concat(self.enc_lstm_outputs , 1),[self.batch_size,self.frame_steps,self.dim_hidden])
        
        ## Decoding stage
        ## Training util
        def train_cap(input_lstm,real_ans,prev_decoder_output,prev_attention_output,prev_state):
           word_index = tf.cond(self.scheduled_sampling_prob >= tf.random_uniform([], 0, 1),
                                lambda: real_ans,
                                lambda: tf.argmax(prev_decoder_output, axis=1))
        ...
        def decode_one_step(prev_output, prev_attention):
            with tf.variable_scope('att_lstm'):
                tf.get_variable_scope().reuse_variables()
                output1, self.att_state = train_cap(
                    att_lstm, self.caption[:, i], prev_output, prev_attention, self.att_state)
            ...
            # attention_output = self.local_attention(output2,enc_lstm_outputs,wp,vp,wa)
            concat_output = tf.concat([attention_output,output2] , 1)
            return tf.nn.xw_plus_b(attention_output, self.w_word_onehot, self.b_word_onehot), tf.tanh(tf.matmul(concat_output,wc))  

        
        ## Decoding stage
        for i in range(caption_steps):
            self.decoder_output, self.attention_output = decode_one_step(self.decoder_output, self.attention_output)
            dec_lstm_outputs.append(self.decoder_output)
