...
class S2VT_model():
    ...
    def __init__(self, batch_size=20, frame_steps=80, frame_feat_dim=4096, caption_steps=30, vocab_size=3000, dim_hidden=300):
        ...
        self.caption = tf.placeholder(tf.float32, [batch_size, caption_steps+1])
        ...
        ## two lstm param
        with variable_scope("att_lstm"):
            att_lstm = tf.contrib.rnn.LSTMCell(dim_hidden)
        with variable_scope("cap_lstm"):
            cap_lstm = tf.contrib.rnn.LSTMCell(dim_hidden)            
        
        att_state = tf.zeros([batch_size, dim_hidden])
        cap_state = tf.zeros([batch_size, dim_hidden])
        
        padding = tf.zeros([batch_size, dim_hidden])
        
        ##################### Computing Graph ########################
        
        frame_flat = tf.reshape(frame, [-1, frame_feat_dim])
        frame_embedding = tf.nn.xw_plus_b( frame_flat, w_frame_embed, b_frame_embed )
        frame_embedding = tf.reshape(frame_embedding, [batch_size, frame_steps, frame_feat_dim])        
        ...
        output = tf.reshape(tf.concate(cap_lstm_outputs , 1), [-1, dim_hidden])                
        ...
        loss = tf.contrib.legacy_seq2seq.sequence_loss_by_example([onehot_word_logits],
                                                                  [tf.reshape(caption[1:], [-1])],
                                                                  [tf.reshape(caption_mask[1:], [-1])])
