...
class GAN_model():
    def __init__(self, z_dim=100, batch_size=100, learning_rate=0.0002, img_shape=(96, 96, 3)):
        self.z_dim = z_dim
    ...
    def _generator(self, z, train_phase, scope_name='generator'):
        ...
        with tf.variable_scope(scope_name) as scope:
            #       z_fc_layer(name, input, n_neuron, img_size, activation, train_phase), output_shape = (1024, 6, 6)
            h = ops.z_fc_layer('z', self.z_vec, shrink_size*shrink_size*1024, shrink_size, tf.nn.relu, self.train_phase)
            
            shrink_size *= 2
            #       transpose_conv_layer(name, input , ksize, output_shape, strides, train_phase), output_shape = (512, 12, 12)
            h = ops.transpose_conv_layer('conv1', h, [5 ,5 , 512, 1024], [self.batch_size, shrink_size, shrink_size, 512], [1,2,2,1], tf.nn.relu, self.train_phase)
            
            #output_shape = (256, 24, 24)
            shrink_size *= 2
            h = ops.transpose_conv_layer('conv2', h, [5 ,5 , 256, 512], [self.batch_size, shrink_size, shrink_size, 256], [1,2,2,1], tf.nn.relu, self.train_phase)
            
            #output_shape = (128, 48, 48)
            shrink_size *= 2
            h = ops.transpose_conv_layer('conv3', h, [5 ,5 , 128, 256], [self.batch_size, shrink_size, shrink_size, 128], [1,2,2,1], tf.nn.relu, self.train_phase)
            
            #output_shape = (64, 96, 96)
            shrink_size *= 2
            gen_img = ops.transpose_conv_layer('gen', h, [5 ,5 , 3, 128], [self.batch_size, shrink_size, shrink_size, 3], [1,2,2,1], tf.nn.tanh, self.train_phase)
    ...
    def _discriminator(self, input_img, train_phase, scope_name='discriminator', reuse=False):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            #       conv_layer(name, input , ksize, strides, do_bn, activ   ation, train_phase)
            h = ops.conv_layer('conv1', input_img, [5, 5, 3,64], [1,2,2,1], False, ops.leaky_relu, self.train_phase)
            h = ops.conv_layer('conv2', h, [5, 5, 64, 128], [1,2,2,1], True, ops.leaky_relu, self.train_phase)
            h = ops.conv_layer('conv3', h, [5, 5, 128, 256], [1,2,2,1], True, ops.leaky_relu, self.train_phase)
            h = ops.conv_layer('conv4', h, [5, 5, 256, 512], [1,2,2,1], True, ops.leaky_relu, self.train_phase)
            
            flatten = tf.reshape(h, [self.batch_size, -1])
            
            #            rf_fc_layer(name, input, n_neuron)
            h_pred = ops.rf_fc_layer('discr', flatten, 1)
    ...
    def _visualize_model(self):
        ...
        print(images.shape)
        ops.save_imshow_grid(images, 'result/', "generated_%d.png" % self.global_steps, shape)