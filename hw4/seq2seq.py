...
def _sample_from_prob_and_embed(embedding, output_projection=None, update_embedding=True):
  def loop_function(prev, _):
    if output_projection is not None:
      prev = nn_ops.xw_plus_b(
          prev, output_projection[0], output_projection[1]
      )
      prob = nn_ops.softmax(prev)
      sample_result = random_ops.multinomial(prob, 1)
      emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)
      if not update_embedding:
        emb_prev = array_ops.stop_gradient(emb_prev)
    return emb_prev
...
def embedding_attention_decoder(decoder_inputs, initial_state, attention_states,
                                ...
                                initial_state_attention=False, feed_type='normal', beam_size=10):
  if output_size is None:
    output_size = cell.output_size
  if output_projection is not None:
    proj_biases = ops.convert_to_tensor(output_projection[1], dtype=dtype)
  ...
  with variable_scope.variable_scope(scope or "embedding_attention_decoder", dtype=dtype) as scope:
    ...
    if feed_type == 'beam':
        ...
        return beam_rnn_decoder(emb_inp, initial_state, cell,
                                ...
                                beam_size=beam_size,
                                scope=scope)
    elif feed_type == 'sample':
        loop_function = _sample_from_prob_and_embed(embedding, output_projection, update_embedding_for_previous)
