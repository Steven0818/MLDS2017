...
class grl_model(object):
    def __init__(self, grl_config, num_samples=512, forward=False, beam_search=False, dtype=tf.float32):
        self.buckets = grl_config.buckets
        ...
        self.num_layers = grl_config.num_layers
        self.max_gradient_norm = grl_config.max_gradient_norm
    ...
    def build_training_graph(self, name_scope):
        print("Building training graph....", end='')
        
        with tf.name_scope("GRL_Cell"):
            single_cell = tf.contrib.rnn.GRUCell(self.emb_dim)
            cells = tf.contrib.rnn.MultiRNNCell([single_cell] * self.num_layers)

        self.s2s_global_step = tf.Variable(0, trainable=False)
        self.rl_global_step = tf.Variable(0, trainable=False)
        ...
        if self.num_samples > 0 and self.num_samples < self.vocab_size:
            w_t = tf.get_variable("proj_w", [self.vocab_size, self.emb_dim], dtype=self.dtype)
            w = tf.transpose(w_t)
        ...
        with tf.name_scope("s2s_Gradient"):
            ...
            for b in range(len(self.buckets)):
                ...
                clips_gradient, norm = tf.clip_by_global_norm(gradients, self.max_gradient_norm)
    ...
    def train(self, loader, iteration=100000):
        loader.start_loaders(n=4)
        reverse_worddict = {v:k for k, v in loader.worddict.items()}

        for it in range(iteration):
            bucket_id, x, mask = loader.get()
    ...
    def _test(self, reverse_worddict, iter_count):
        ...
        input_feed = {self.forward_only.name: True}
        encoder_size, decoder_size = self.buckets[0]

        for i in range(encoder_size):
            input_feed[self.encoder_inputs[i].name] = sent_list[:,i]
        
        for i in range(decoder_size):
                input_feed[self.decoder_inputs[i].name] = np.ones([4], dtype=np.int32)
                input_feed[self.target_weights[i].name] = np.ones([4], dtype=np.int32)
        for i in range(len(self.buckets)):
                input_feed[self.rewards[i].name] = 1.
...
"""
    def step(self, session, encoder_inputs, decoder_inputs, target_weights, reward, bucket_id, forward_only, beam_sㄛearch):
        encoder_size, decoder_size = self.buckets[bucket_id]
        ...
        if not forward_only:
            ...
            output_feed = [self.losses[bucket_id], self.encoder_states[bucket_id], self.outputs[bucket_id]]
    ...
    def step_rl(self, session, st_model, cc_model, bk_model, encoder_inputs, decoder_inputs, target_weights,
                batch_source_encoder, bucket_id,beam_search=False):
        ...
        while True:
            ...
            step_loss, encoder_states, output_logits = self.step(session, encoder_inputs, decoder_inputs, target_weights,
                                                                 reward=1, bucket_id=bucket_id, forward_only=True,beam_search = False)
    ...
    def logProb(self, session, model, buckets, tokens_a, tokens_b, mask=None):
        ...
        # step
        output_logits, _ = model.step(session, encoder_inputs, decoder_inputs, target_weights,
        ...
        #print ("remove_type type=1 tokens: %s" %sequence)
            ...
            for seq in sequence:
                 ...
                 for t in seq:
                     ...
                     token.append(np.argmax(t, axis=0).astype(int))
    ...
    def get_batch(self, train_data, bucket_id, type=0):
        ...
        for batch_i in range(self.batch_size):
            if type == 1:
                # feed_data = {bucket_id: zip(tokens_a, tokens_b)}
                #print (train_data[bucket_id][batch_i])
                if batch_i ==0:
                    type1_unzip = list(train_data[bucket_id])
                encoder_input, decoder_input = type1_unzip[batch_i]
